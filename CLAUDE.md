...
- **Skills:** ONE definition (the "reusable part of an agent" one, in the frontmatter description only). Create = from agent chat OR Skills section (Create in Chat / Create Manually / Import Skill) — shown as Tabs. Use = reference the skill explicitly in the agent. Share = Personal / Team / Org. Skills-vs-knowledge only as a short bottom note.
